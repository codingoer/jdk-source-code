...
public class BooleanAnd extends BinaryExpr {
   protected BooleanAnd(Expression var1, Expression var2) {
      super("&&", var1, var2);
   }
      ...
      try {
         ...
         if (var1 instanceof Number) {
            if (var1 instanceof BigInteger) {
               var3 = new Boolean(((BigInteger)var1).compareTo(BigInteger.valueOf(0L)) != 0);
            } else {
         ...
         if (var6 instanceof Number) {
            if (var6 instanceof BigInteger) {
               var4 = new Boolean(((BigInteger)var6).compareTo(zero) != 0);
            } else {
         ...
         String[] var2 = new String[]{Util.getMessage("EvaluationException.booleanAnd"), this.left().value().getClass().getName(), this.right().value().getClass().getName()};
