...
public class SOAPEntityReferenceValidator implements EntityReferenceValidator {
   private static final Set _validTypes = new HashSet();
   ...
   public boolean isValid(Kind kind, QName name) {
      if (name.getNamespaceURI().equals("http://www.w3.org/XML/1998/namespace")) {
         return true;
      } else if (kind == SchemaKinds.XSD_TYPE) {
         return _validTypes.contains(name);
      } else if (kind == SchemaKinds.XSD_ELEMENT) {
         return _validElements.contains(name);
      } else {
         return kind == SchemaKinds.XSD_ATTRIBUTE ? _validAttributes.contains(name) : false;
      }
   }
