...
public final class Names {
   private static final Map RESERVED_WORDS = new HashMap(53);

   private Names() {
   }

   public static String getPortName(Port port) {
      String javaPortName = (String)port.getProperty("com.sun.xml.internal.ws.processor.model.JavaPortName");
      if (javaPortName != null) {
         return javaPortName;
      } else {
         QName portName = (QName)port.getProperty("com.sun.xml.internal.ws.processor.model.WSDLPortName");
         if (portName != null) {
   ...
   public static String getJavaReserverVarialbeName(@NotNull String name) {
      return RESERVED_WORDS.get(name) == null ? name : (String)RESERVED_WORDS.get(name);
   }
