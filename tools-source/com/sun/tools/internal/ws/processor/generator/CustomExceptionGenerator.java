...
public class CustomExceptionGenerator extends GeneratorBase {
   ...
   private boolean isRegistered(Fault fault) {
      if (this.faults.keySet().contains(fault.getJavaException().getName())) {
         fault.setExceptionClass((JClass)this.faults.get(fault.getJavaException().getName()));
         return true;
      } else {
         return false;
      }
   ...
   private void registerFault(Fault fault) {
      try {
         ...
         throw new GeneratorException("generator.nestedGeneratorError", new Object[]{var3});
      }
   }
   ...
   private void write(Fault fault) throws JClassAlreadyExistsException {
      ...
      Iterator var5 = this.getJAXWSClassComment().iterator();

      while(var5.hasNext()) {
         String doc = (String)var5.next();
         comment.add(doc);
      }
      ...
      cb1.invoke("super").arg((JExpression)var1);
      cb1.assign(fr, var2);
      ...
      cb2.invoke("super").arg((JExpression)var1).arg((JExpression)var3);
      cb2.assign(fr, var2);
      JMethod fim = cls.method(1, (JType)faultBean, "getFaultInfo");
